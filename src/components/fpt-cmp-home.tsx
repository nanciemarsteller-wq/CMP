  ...
  return <><Header /><main>
    ...
    <section className="section solution" id="giai-phap" aria-labelledby="solution-title"><div className="container">
      ...
      {(() => { const tab = solutionTabs[solutionTab]!; return <div className="solution-panel" role="tabpanel" id="solution-panel" aria-labelledby={`solution-tab-${solutionTab}`}>
        <div className="solution-copy"><h3>{tab.heading}</h3><p>{tab.text}</p></div>
    ...
    <section className="section features" id="tinh-nang"><div className="container"><div className="section-heading"><p className="eyebrow">Tính năng cốt lõi</p><h2>Kiểm soát toàn diện quản trị sự đồng ý</h2><p>Tập trung quản lý sự đồng ý, chủ động phát hiện rủi ro và đảm bảo khả năng truy vết trên toàn bộ hệ sinh thái số.</p></div>
      <div className="feature-list">{features.map((feature)=><article className={`feature-row ${feature.highlighted ? "feature-highlight" : ""}`} key={feature.title}><div className="feature-copy"><h3>{feature.title}</h3><p>{feature.description}</p><ul>{feature.bullets.map((item)=><li key={item}><Check />{item}</li>)}</ul>{feature.cta && <Button onClick={() => scrollTo("dang-ky-demo")}>Đăng ký xem demo <ArrowRight size={18} /></Button>}</div><ProductImage src={feature.image} alt={feature.alt} onOpen={openImage} /></article>)}</div>
    </div></section>
